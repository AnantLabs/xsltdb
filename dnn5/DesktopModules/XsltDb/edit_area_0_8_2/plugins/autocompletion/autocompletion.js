...
var EditArea_autocompletion = {
    ...
    */
	, onkeydown: function(e) {
	    ...
	    if (this._isShown()) {
	        ...
	        else if (letter == "Tabulation" || letter == "Down") {
	            this._selectNext();
	            return false;
	        }
	        else if (letter == "Page down") {
	            this._selectNext('page');
	            return false;
	        }
	        else if (letter == "Up") {
	            this._selectBefore();
    ...
    */
	...
	, _selectNext: function(isPage) {
	    ...
	    this.selectIndex += isPage ? 10 : 1;
	    this.selectIndex = (this.selectIndex >= as.length || this.selectIndex < 0) ? 0 : this.selectIndex;

	    var bot = 0;
	    for (var j = 0; j <= this.selectIndex && j < as.length; j++)
	        bot += as[j].clientHeight;

	    var a = as[this.selectIndex];
	    a.className += " focus";

	    this.showhelp(a);

	    var scroll = this.container.firstChild;
	    var maxHeight = scroll.clientHeight;

	    var top = bot - a.clientHeight;
	    if (bot > scroll.scrollTop + maxHeight)
	        scroll.scrollTop = bot - maxHeight;
	    if (top < scroll.scrollTop)
	        scroll.scrollTop = top;
	}
    // select the previous element in the suggested box
	, _selectBefore: function(isPage) {
	    var as = this.container.getElementsByTagName('A');
    ...
    */
	        ...
	        if (!parent.editAreaLoader.syntax[lang]['autocompletion'])	// init the regexp if not already initialized
	        {
	            ...
	            if (parent.editAreaLoader.load_syntax[lang]['AUTO_COMPLETION']) {
	                ...
	                for (var i in parent.editAreaLoader.load_syntax[lang]['AUTO_COMPLETION']) {
	                    ...
	                    tmp["match_pref"] = new RegExp("(?:" + datas["REGEXP"]["before_word"] + ")(" + datas["REGEXP"]["possible_words_letters"] + ")(?:" + datas["REGEXP"]["prefix_separator"] + ")$", tmp["modifiers"]);
	    ...
	    if (editArea.is_editable) {
	        ...
	        for (var i in this.curr_syntax) {
	            ...
	            if (matchNextletter || this.forceDisplay) {
	                ...
	                // we'll display to list of all available words
	                    ...
	                    //[AB]
	                    //for(var prefix in this.curr_syntax[i]["keywords"])
	                    var prefix = last_chars.match(this.curr_syntax[i]["match_pref"]);
	                    if (prefix) prefix = prefix[1];
	                    if (this.curr_syntax[i]["keywords"][prefix])
	                    //[/AB]
	                    {
	                        for (var j = 0; j < this.curr_syntax[i]["keywords"][prefix]['datas'].length; j++) {
	                            hasMatch = false;
	        ...
	        if (results.length == 1 && this.autoSelectIfOneResult) {
	            ...
	            for (var i = 0; i < results.length; i++) {
	                var line = "<li><a href=\"#\" class=\"entry\" onmouseover=\"EditArea_autocompletion.showhelp(this)\" onmouseout=\"EditArea_autocompletion.clearhelp()\" onmousedown=\"EditArea_autocompletion._select('" + results[i][1]['replace_with'].replace(new RegExp('"', "g"), "&quot;") + "');return false;\">" + results[i][1]['comment'];
	            ...
	            // sort results
	            this.container.innerHTML = '<div style="height:160px;overflow:auto;"><ul>' + lines.sort().join('') + '</ul></div>' + '<div class="help"></div>';
	...
	, showhelp: function(a) {
	    var help = this.container.firstChild.nextSibling;
	    if (a.firstChild.getAttribute) help.innerHTML = a.firstChild.getAttribute('help') + (a.firstChild.getAttribute('url') ? " <span class='more' onmousedown='EditArea_autocompletion.openhelp(\"" + a.firstChild.getAttribute('url') + "\")'>more</span>" : "");
	    else help.innerHTML = "";
	}
	, clearhelp: function() {
	    if (this.selectIndex >= 0) {
	        var as = this.container.getElementsByTagName('A');
	        if (this.selectIndex < as.length)
	            this.showhelp(as[this.selectIndex]);
	        else
	            this.container.firstChild.nextSibling.innerHTML = "";
	    }
	    else
	        this.container.firstChild.nextSibling.innerHTML = "";
	}
