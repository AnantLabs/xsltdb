...
namespace Findy.XsltDb
{
    ...
    public class XsltDbUtils
    {
        ...
        {
            aConfig conf = null;
            using (IDataReader r = Data.ExecuteReader("Findy_XsltDb_Select", ModuleID))
                if (r.Read())
                    conf = new aConfig(
                        r["XSLT"].ToString(),
                        r["Draft"].ToString(),
        ...
        {
            aConfig conf = null;
            using (IDataReader r = Data.ExecuteReader("Findy_XsltDb_GetServiceConfig", PortalController.GetCurrentPortalSettings().PortalId, ServiceName))
                if (r.Read())
                    conf = new aConfig(
                        r["XSLT"].ToString(),
                        r["Draft"].ToString(),
        ...
        public static aConfig GetConfig(int ModuleID, int TabID)
        {
            ...
            if (tabConfigs == null)
            {
                ...
                using (IDataReader r = Data.ExecuteReader("Findy_XsltDb_Select_AllForTab", TabID))
                    while (r.Read())
        ...
        {
            Data.ExecuteNonQuery("Findy_XsltDb_AttachModuleToConfig", ModuleID, ConfigID);
        }
        public static DataTable GetConfigs()
        {
            DataTable dt = new DataTable("conf");
            using (IDataReader r = Data.ExecuteReader("Findy_XsltDb_GetAllConfigs"))
                dt.Load(r);
            return dt;
        }
        ...
        public static DataTable GetModulesWithSameConfig(int ModuleID)
        {
            ...
            using (IDataReader r = Data.ExecuteReader("Findy_XsltDb_GetModulesWithSameConfig", ModuleID))
                dt.Load(r);
            return dt;
        }
        ...
        {
            using (IDataReader r = Data.ExecuteReader("Findy_XsltDb_GetModuleData", ModuleID))
                if (r.Read())
                    return r[0].ToString();
            return "<xsltdb/>";
        }
        ...
        {
            Data.ExecuteNonQuery("Findy_XsltDb_ImportModule", ModuleID, data);
        }

        public static void SaveXSLT(int ModuleID, string xslt, bool isSuper, string Name, string ServiceName)
        {
            Data.ExecuteNonQuery("Findy_XsltDb_Save", ModuleID, isSuper ? 1 : 0, xslt, Name, ServiceName);
            StaticCache.Clear();
        }
        ...
        {
            Data.ExecuteNonQuery("Findy_XsltDb_Publish", ModuleID);
        }
...
";
            ...
            using (IDataReader dr = Data.ExecuteSQL(sql))
                dt.Load(dr);
        ...
        public static XsltDbPortalSettings GetPortalSettings(int PortalID)
        {
            ...
            using (IDataReader r = Data.ExecuteReader("Findy_XsltDb_SelectPortalSettings", PortalID))
            {
                if (r.Read())
                {
...
";
        ...
        private const string xslSmallHeaderOutput = @"<xsl:output method=""html"" indent=""yes"" omit-xml-declaration=""yes""/>";
...
";
        ...
        static Regex ServiceRegex = new Regex(@"<\s*mdo:service\s+name\s*=\s*""([^""]+)""\s*(type\s*=\s*""[^""]+""\s*)?>(.*?)<\s*/\s*mdo:service\s*>", RegexOptions.Singleline);
        static Regex ServiceTypeRegex = new Regex(@"""(.+)""", RegexOptions.Singleline);
        ...
        public string ExecuteService(string xsl, string xml, string serviceName, bool IsSuper, out string type)
        {
            type = "text/xml";
            ...
            {
                if (string.Compare(HttpUtility.HtmlDecode(m.Groups[1].Value), serviceName, true) == 0)
                {
                    if (m.Groups[2].Success)
                        type = HttpUtility.HtmlDecode(ServiceTypeRegex.Match(m.Groups[2].Value).Groups[1].Value);
                    return JustTransform(m.Groups[3].Value, xml, false, IsSuper);
                }
            }
        ...
        public string Transform(string xml, string xsl, bool IsSuper)
        {
            ...
            Regex r = new Regex(@"<\s*(mdo:)?callable\s+js\s*=\s*""([^\(]+)\(([^\)]*)\)\""\s*>(.*?)<\s*/\s*(mdo:)?callable\s*>", RegexOptions.Singleline);
...
";
        ...
        public static string PrepareXslt(string xsl)
        {
            ...
            {
                int templ = xsl.IndexOf("<xsl:template");
                if (templ < 0)
                    xsl = string.Format(xslLargeHeaderFooter, xsl);
                else
                {
                    if (xsl.IndexOf("<xsl:output") < 0)
                    {
                        
                        xsl = 
                            xsl.Substring(0, templ) +
                            xslSmallHeaderOutput +
                            xsl.Substring(templ);
                    }
        ...
        public static string ReplaceValueOf(string xsl)
        {
            ...
            r = new Regex("<\\s*xsl:execute\\s+select=\"([^\"]*?)\"\\s*/\\s*>");
            xsl = r.Replace(xsl, "<xsl:if test=\"$1\" />");

            xsl = Regex.Replace(xsl, @"#\s*([\w\.-]+)\s*/\s*([\w\.-]+)\s*#", @"mdo:coalesce($$${1}/${2}/value[@culture=mdo:culture()], $$${1}/${2}/value[position()=1])");
